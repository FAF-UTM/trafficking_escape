...
const CombinationLock: React.FC<CombinationLockProps> = ({ onComplete }) => {
    ...
    const handleClueClick = () => {
        openModal(
            "/assets/trafficking_escape_clue.png",
            ...
            {stage === "intro" && (
                <div className="intro-screen fade-in">
                    ...
                    <p className="intro-text">
                        You find yourself confined in a dim, abandoned room—a stark reminder of exploitation.
                        In the distance, a heavy door stands between you and the hope of freedom. Embedded on the door is a combination lock,
                        its numbers shrouded in mystery.
                    </p>
                    <p className="intro-text">
                        A nearly faded note clings to the wall. It hints at a secret sequence.
                        Will you trust your instincts and decode the clue to escape?
                    </p>
            ...
            {stage === "puzzle" && (
                <div className="puzzle-stage fade-in">
                    ...
                    <div className="clue-zone" onClick={handleClueClick}>
                        {/* Optionally add a clue icon here */}
                    </div>
                    {/* Code Display */}
                    ...
                    <div className="keypad-container">
                        {["1", "2", "3", "4", "5", "6", "7", "8", "9"].map(num => (
                            <button key={num} className="keypad-button bounce" onClick={() => handleButtonClick(num)}>
                                {num}
                            </button>
                        ))}
                        <button className="keypad-button" onClick={handleClear}>Clear</button>
                        <button className="keypad-button" onClick={() => handleButtonClick("0")}>0</button>
                        <button className="keypad-button" onClick={handleSubmit}>Enter</button>
                    </div>
                </div>
            )}
            ...
            {stage === "success" && (
                <div className="success-screen fade-in">
                    ...
                    <p className="success-text">
                        With a resonant click, the lock yields. The door creaks open, ushering in a ray of hope and the promise of freedom.
                    </p>
